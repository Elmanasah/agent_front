...
export default function Canvas({ content, isOpen, onClose, isWriting }) {
    ...
    return (
        <div className="flex flex-col h-full bg-[var(--surface)] border-l border-[var(--border)] glass animate-fade-in w-[45%] min-w-[400px]">
            {/* Header */}
                ...
                <div className="flex items-center gap-2">
                    ...
                    {isWriting && (
                        <div className="flex gap-1 ml-2">
                            <div className="w-1 h-1 bg-emerald-400 rounded-full dot dot-1"></div>
                            <div className="w-1 h-1 bg-emerald-400 rounded-full dot dot-2"></div>
                            <div className="w-1 h-1 bg-emerald-400 rounded-full dot dot-3"></div>
                        </div>
                    )}
                </div>
                <button 
            ...
            {/* Content Area */}
            <div className="flex-1 overflow-auto p-6 text-[var(--text-main)] leading-relaxed font-mono text-sm selection:bg-indigo-500/30">
                {!content && !isWriting ? (
            ...
            <div className="p-3 border-t border-[var(--border)] flex justify-end gap-2 bg-black/5 dark:bg-black/20">
                <button className="text-[10px] px-3 py-1 bg-white/5 hover:bg-white/10 border border-[var(--border)] rounded-full text-[var(--text-muted)] transition-all">
                    COPY CONTENT
                </button>
                <button className="text-[10px] px-3 py-1 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/20 rounded-full text-indigo-400 transition-all font-medium">
                    DOWNLOAD .TXT
                </button>
            </div>
        </div>
