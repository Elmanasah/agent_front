...
const TABS = [
    { id: 'account',  label: 'My Account',  icon: <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="10" r="3"/><path d="M7 20.662V19a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v1.662"/></svg> },
    { id: 'usage',    label: 'Usage',        icon: <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 3v18h18"/><path d="m19 9-5 5-4-4-3 3"/></svg> },
    { id: 'security', label: 'Security',     icon: <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg> },
...
function ResourceGauge({ label, icon, color, used = 0, limit = 0 }) {
    ...
    const barColor = danger ? 'bg-rose-500' : warning ? 'bg-amber-500' : `bg-${color}-500`;
...
export default function Settings() {
    ...
    useEffect(() => {
        ...
        setPassword(''); setCurrentPassword(''); setNewPassword('');
    }, [user]);
    ...
    const handleSave = async (e) => {
        e.preventDefault(); setError(null); setSuccessMsg(null); setIsSaving(true);
        try { await updateMe({ name, phone }); setSuccessMsg('Profile updated!'); setTimeout(() => setSuccessMsg(null), 3000); }
    ...
    const handlePasswordChange = async (e) => {
        e.preventDefault(); setError(null); setSuccessMsg(null); setIsSaving(true);
        if (!currentPassword || !newPassword) { setError('Both current and new passwords are required.'); setIsSaving(false); return; }
        try { await changePassword(currentPassword, newPassword); setSuccessMsg('Password changed!'); setCurrentPassword(''); setNewPassword(''); setTimeout(() => setSuccessMsg(null), 3000); }
    ...
    const handleDelete = async (e) => {
        ...
        setIsDeleting(true);
        try { await deleteMe(password); }
        catch (err) { setError(err.response?.data?.message || 'Failed to delete account.'); setIsDeleting(false); }
    };
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center sm:py-10 sm:px-4 py-0 px-0">
            <div className="w-full h-full sm:h-auto max-w-2xl bg-white dark:bg-slate-900 sm:rounded-2xl rounded-none sm:shadow-xl shadow-none sm:border border-none border-slate-200 dark:border-white/10 flex flex-col overflow-hidden animate-fade-in">
                ...
                <div className="flex items-center gap-4 px-5 sm:px-8 py-5 sm:py-6 border-b border-slate-200 dark:border-white/10 shrink-0">
                    <button onClick={() => navigate(-1)} className="w-10 h-10 shrink-0 rounded-xl bg-slate-100 dark:bg-white/5 flex items-center justify-center text-slate-500 hover:text-slate-900 dark:hover:text-white transition-all">
                    ...
                    <div className="min-w-0">
                        ...
                        <p className="text-[12px] sm:text-[13px] text-slate-500 dark:text-slate-400 mt-0.5">Manage your account preferences and security.</p>
                    </div>
                </div>
                    ...
                    <div className="flex-1 p-5 sm:p-6 md:p-8 overflow-y-auto">
                            ...
                            <div className="animate-fade-in space-y-6 max-w-md">
                                ...
                                <form onSubmit={handleSave} className="space-y-5">
                                    <div>
                                        ...
                                        <input type="email" value={email} disabled className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-slate-500 text-[14px] cursor-not-allowed" />
                                    </div>
                                    <div>
                                        <label className="block text-[12px] font-bold text-slate-500 uppercase tracking-widest mb-2">Full Name</label>
                                        <input type="text" value={name} onChange={e => setName(e.target.value)} required className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-white/10 bg-transparent text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all text-[14px]" />
                                    </div>
                                    <div>
                                        <label className="block text-[12px] font-bold text-slate-500 uppercase tracking-widest mb-2">Phone Number</label>
                                        <input type="tel" value={phone} onChange={e => setPhone(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-white/10 bg-transparent text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all text-[14px]" placeholder="+1 (555) 000-0000" />
                                    </div>
                                    <button type="submit" disabled={isSaving} className="w-full py-3 px-4 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white text-[14px] font-bold rounded-xl transition-all shadow-md">
                        ...
                        {activeTab === 'usage' && (
                            <div className="animate-fade-in space-y-6">
                                <div>
                                    <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-1">Usage & Quota</h3>
                                    <p className="text-[13px] text-slate-500 dark:text-slate-400">Your current plan usage and activity history.</p>
                                </div>
                                ...
                                {usage?.isLocked && (
                                    <div className="p-4 rounded-xl bg-rose-500/10 border border-rose-500/20 flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-[13px] font-bold text-rose-400">Account Restricted</p>
                                            <p className="text-[12px] text-rose-400/70 mt-0.5">{usage.lockReason || 'Contact support for assistance.'}</p>
                                        </div>
                                    </div>
                                )}
                                ...
                                {usageLoading ? (
                                    ...
                                    <div className="p-4 rounded-2xl bg-slate-50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/10 flex flex-wrap gap-6 items-center">
                                        <div>
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Current Plan</p>
                                            <span className={`text-[12px] font-bold px-3 py-1 rounded-full border uppercase tracking-wide ${PLAN_COLORS[usage.plan] || PLAN_COLORS.free}`}>
                                                {usage.plan}
                                            </span>
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[13px] font-semibold text-slate-700 dark:text-slate-200">
                                                {usage.nextResetAt ? new Date(usage.nextResetAt).toLocaleDateString() : '—'}
                                            </p>
                                        </div>
                                    </div>
                                ) : null}
                                ...
                                {usageLoading ? (
                                    <div className="grid grid-cols-2 gap-3">
                                        {[...Array(4)].map((_, i) => <div key={i} className="h-24 rounded-2xl bg-slate-100 dark:bg-white/5 animate-pulse" />)}
                                    </div>
                                ) : usage?.resources ? (
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        {RESOURCE_META.map(({ key, label, icon, color }) => (
                                            <ResourceGauge key={key} label={label} icon={icon} color={color} {...usage.resources[key]} />
                                        ))}
                                    </div>
                                ) : null}
                                ...
                                <div className="rounded-2xl border border-dashed border-slate-300 dark:border-white/10 p-5 flex items-start gap-4">
                                    <div className="w-10 h-10 shrink-0 rounded-xl bg-slate-100 dark:bg-white/5 flex items-center justify-center text-slate-400 text-lg">💳</div>
                                    <div>
                                        <p className="text-[14px] font-bold text-slate-700 dark:text-slate-200 mb-1">Billing & Payments</p>
                                        <p className="text-[13px] text-slate-500 dark:text-slate-400 leading-relaxed">
                                            We're working on a billing portal. Paid plans will be available soon.
                                        </p>
                        ...
                        {activeTab === 'security' && (
                            <div className="animate-fade-in space-y-6 max-w-md">
                                <div>
                                    ...
                                    <p className="text-[13px] text-slate-500 dark:text-slate-400">Use a long, random password to stay secure.</p>
                                </div>
                                <form onSubmit={handlePasswordChange} className="space-y-5">
                                    <div>
                                        <label className="block text-[12px] font-bold text-slate-500 uppercase tracking-widest mb-2">Current Password</label>
                                        <input type="password" value={currentPassword} onChange={e => setCurrentPassword(e.target.value)} required className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-white/10 bg-transparent text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all text-[14px]" placeholder="Current password" />
                                    </div>
                                    <div>
                                        <label className="block text-[12px] font-bold text-slate-500 uppercase tracking-widest mb-2">New Password</label>
                                        <input type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)} required className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-white/10 bg-transparent text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all text-[14px]" placeholder="New password" />
                                    </div>
                                    <button type="submit" disabled={isSaving || !currentPassword || !newPassword} className="w-full py-3 px-4 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white text-[14px] font-bold rounded-xl transition-all shadow-md">
                                ...
                                <form onSubmit={handleDelete} className="space-y-5">
                                    <div className="p-5 rounded-xl border border-rose-500/30 bg-rose-500/5">
                                        ...
                                        <p className="text-slate-600 dark:text-slate-400 text-[13px] leading-relaxed">
                                            Once you delete your account, there is no going back. All of your conversations, knowledge bases, and generated content will be permanently wiped from our servers.
                                        </p>
                                    </div>
                                    <div>
                                        <label className="block text-[12px] font-bold text-slate-500 uppercase tracking-widest mb-2">Confirm Current Password</label>
                                        <input type="password" value={password} onChange={e => setPassword(e.target.value)} required className="w-full px-4 py-3 rounded-xl border border-rose-200 dark:border-rose-500/30 bg-transparent text-slate-900 dark:text-white focus:outline-none focus:border-rose-500 focus:ring-1 focus:ring-rose-500 transition-all text-[14px]" placeholder="Enter password to verify" />
                                    </div>
                                    <button type="submit" disabled={isDeleting || !password} className="w-full py-3 px-4 bg-rose-600 hover:bg-rose-700 disabled:opacity-50 text-white text-[14px] font-bold rounded-xl transition-all shadow-md">
