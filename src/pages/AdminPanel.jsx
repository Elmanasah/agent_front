...
export default function AdminPanel() {
    ...
    // AuthContext may expose `id` or `_id` — handle both
    ...
    const handleEditSave = async (e) => {
        ...
        setEditError(null);
        try {
            const updated = await UserService.updateUser(editingUser._id, editForm);
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm overflow-hidden">
                    ...
                    {loading ? (
                        <div className="flex items-center justify-center py-16 text-slate-400">
                            <svg className="w-5 h-5 animate-spin mr-2" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/></svg>
                            Loading users...
                        </div>
                    ) : filtered.length === 0 ? (
                        <div className="py-16 text-center text-slate-400 dark:text-slate-500 text-[13px]">No users found.</div>
                    ) : (
                                            ...
                                            <td className="px-5 py-3.5">
                                                <div className="flex items-center justify-end gap-2">
                                                    ...
                                                    {u.id !== myId ? (
                                                        ...
                                                        <span className="text-[11px] text-slate-400 dark:text-slate-500 italic font-medium">You</span>
                                                    )}
                                                </div>
                                            </td>
                                        </tr>
            ...
            {editingUser && (
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[200] flex items-center justify-center p-4 animate-fade-in">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-white/10 w-full max-w-sm p-6 animate-fade-in">
                        ...
                        {editError && (
                            <div className="mb-4 p-3 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-500 text-[12px]">{editError}</div>
                        )}
